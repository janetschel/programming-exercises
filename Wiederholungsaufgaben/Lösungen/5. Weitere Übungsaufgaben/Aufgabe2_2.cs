...
namespace Weitere_Uebungsaufgaben
{
    class Aufgabe2_2
    {
        static void Main(string[] args)
        {
            ...
            var neededFuel = CalculateNeededFuelForModuleMasses(moduleMasses);
            Console.WriteLine(neededFuel);
        }

        static int CalculateNeededFuelForModuleMasses(int[] moduleMasses)
        {
            var neededFuel = 0;

            foreach (var moduleMass in moduleMasses)
            {
                int neededFuelForModuleMass = moduleMass;
                do
                {
                    neededFuelForModuleMass = neededFuelForModuleMass / 3 - 2;
                    neededFuel += neededFuelForModuleMass;
                } while (neededFuelForModuleMass > 0);
            }
